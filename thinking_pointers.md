...
audio domain problem associated with giving binary response (1-speech,0-silence/noise)
using Silero VAD (./models/silero_vad.onnx) on onnx runtime, and connecting to loudness meter, we get only the loudness metrics of voice activity
since currently we are not using any bundler for packaging into one optimized file, use CDN

importScripts() is the Web Worker equivalent of ```<script>``` in HTML regular scripts can load external scripts but inside a web worker there is no HTML, no DOM
so browsers give workers importScripts() instead - it fetches and executes a script file synchronously inside the worker. 

AudioWorklet runs on the audio rendering thread, which is a dedicated real-time thread managed by the browser's audio engine, it has stricter timing guarantees than a Web Worker and is optimized for audio processing.

main thread      → UI, DOM, script.js
audio thread     → AudioWorklet, process(), real-time audio
web worker       → heavy computation (ONNX inference)

Silero's LSTM has 2 layers and 64 hidden units per layer. these are initialized to zero at the start - meaning the model starts with no memory of previous audio.

LSTM (Long Short Term Memory) - type of RNN (recurrent neural network) designed to remember patterns over time. LSTM carries 'state' between inputs (feedback-system)

chunk_1 → LSTM → output, new_state
chunk_2 → LSTM(new_state) → output, new_state
chunk_3 → LSTM(new_state) → output, new_state

2 state vectors in LSTM
h → hidden state  (short-term memory, what just happened)
c → cell state    (long-term memory, patterns over time)

for VAD, this matter because speech has temporal patterns - a single 32ms chunk doesn't tell you much. But the LSTM remembers the last several chunks and uses that context to decide "this is still speech" vs "this is silence starting"
without LSTM, every chunk would be judged in isolation. With it, the model understands continuity.

thread synchronization is important considering features are divided between worker threads.
to work silero needs a sample rate of 16kHz and chunk size of 512 samples at sr that is nearly 32ms

and mic runs at 48kHz so we need to downsample 3:1 before sending to the worker

calc involved:
48kHz = 48000 samples per second
16kHz = 16000 samples per second

ratio = 48000/16000 = 3
-> for every 3 samples at 48kHz, we keep 1
-> that's our 16kHz sample

512 samples at 16kHz * 3 = 1536 samples at 48kHz
so we collect 1536 raw samples
then take every 3rd one:
index 0, 3, 6, 9 ... 1533 -> totally 512 samples at 16kHz

and that's called Decimating, throwing away 2 out of every 3 samples.
this is why proper downsampling should apply a low-pass filter first (anti-aliasing filter)
...
[Decimation and Interpolation](https://fiveable.me/advanced-signal-processing/unit-5/decimation-interpolation/study-guide/IguzjvxD4cSBH9NO)
